...
class PwrightUtils:
    ...
    def wait_for_changed_text_content(self, selector, timeout=10, timeout_ignore=False):
        ...
        if not self.elements_content.get(selector):
            raise ValueError(f"Initial text content was not stored for selector '{selector}'.")
        end_time = int(time.time()) + timeout
        while int(time.time()) < end_time:
            try:
                expect(self.page.locator(selector)).not_to_have_text(self.elements_content[selector], timeout=1000)
                self.elements_content[selector] = self.page.locator(selector).text_content()
                return True
            except:
        ...
        if timeout_ignore:
            return False
        raise TimeoutError(f"Text content of element '{selector}' was not changed during {timeout} seconds.")
...
class TableUtils:
    ...
    def get_column_value_from_matched_row(self, column_to_text_dict, column_name):
        ...
        """
        matched_rows_indices = self.get_rows_indices_by_values_in_columns(column_to_text_dict)
        ...
        column_value = self.page.locator(self.row_columns_template.format(row_index=matched_rows_indices[0]))\
            .nth(column_index).text_content()
        return column_value
